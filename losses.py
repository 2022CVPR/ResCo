...
class SiamBalSfx_SiamBalQ(nn.Module):
    ...
    def __init__(self, cls_num_list, queue_size_per_cls, balsfx_n=0.0, temperature=1, con_weight=1.0, effective_num_beta=0.999):
        super(SiamBalSfx_SiamBalQ, self).__init__()
    ...
    def forward(self, sim_con, labels_con, logits_cls_q, logits_cls_k, labels):
        logits_cls_q = logits_cls_q + torch.log(self.class_prior + 1e-9)
        logits_cls_k = logits_cls_k + torch.log(self.class_prior + 1e-9)
        loss_cls = 0.5 * self.criterion_cls(logits_cls_q, labels) + 0.5 * self.criterion_cls(logits_cls_k, labels)
